...
while ($row = mysqli_fetch_array($listaProveedores)) {
    ...
    if(($row[10] == '1')||($_SESSION['tipo'] == 0)){
        ...
        if($ut == $row[11]){
            $estadoBtn .='<a style="z-index: 0;color:#000" onclick="bloquearEdicion(\''.$row[7].'\')" href="controlador/imprimirpollopdf.php?id='.$row[7].'" target="_blank"><i class="bi bi-printer fs-2 text-primary"></i></a></center>';
        }        
    }else{
        if($ut == $row[11]){
            $estadoBtn = '<center>
